...
For \emph{quadratic} functionals of $\rho$, such as the purity $\operatorname{Tr}(\rho^2)$ and the R\'enyi-2 entropy $S_2 = -\log_2 \operatorname{Tr}(\rho^2)$, the formalism was extended by Elben \textit{et al.} \cite{elben2020cross}, Brydges \textit{et al.} \cite{brydges2019probing}, and Vermersch \textit{et al.} \cite{vermersch2018statistical}, building on earlier work by van Enk and Beenakker \cite{vanenk2012measuring}. The resulting protocol uses randomized Pauli measurements together with the U-statistic estimator of order two, which is unbiased and exhibits variance scaling $\Var[\widehat{\operatorname{Tr}}(\rho^2)] \sim 4^k / N$, where $k$ is the number of qubits in the measured subsystem and $N$ is the number of measurements.
...
A general (possibly mixed) quantum state is described by a \emph{density operator} $\rho$, which is a positive semidefinite Hermitian operator with unit trace, $\rho \geq 0$, $\rho^\dagger = \rho$, $\operatorname{Tr}(\rho) = 1$ \cite{nielsen2010quantum}. A state is \emph{pure} if and only if $\operatorname{Tr}(\rho^2) = 1$; otherwise it is \emph{mixed}. The degree of mixedness is captured by the purity functional
\begin{equation}
    P_2(\rho) = \operatorname{Tr}(\rho^2), \qquad 1/2^{n} \leq P_2(\rho) \leq 1.
\end{equation}
...
\begin{equation}
    p(m) = \operatorname{Tr}\bigl(M_m^\dagger M_m \rho\bigr),
\end{equation}
...
\begin{equation}
    \rho_A = \operatorname{Tr}_B(\rho_{AB}) = \sum_j (\mathbb{I}_A \otimes \langle j|) \rho_{AB} (\mathbb{I}_A \otimes |j\rangle),
\end{equation}
where $\{|j\rangle\}$ is an orthonormal basis of $\mathcal{H}_B$.

The \emph{purification theorem} states that every mixed state $\rho_A$ on $\mathcal{H}_A$ can be represented as the reduced state of a pure state on a larger Hilbert space: there exists a pure state $|\psi\rangle_{AB} \in \mathcal{H}_A \otimes \mathcal{H}_B$ with $\mathcal{H}_B \cong \mathcal{H}_A$ such that $\rho_A = \operatorname{Tr}_B(|\psi\rangle\langle\psi|_{AB})$ \cite{nielsen2010quantum}. A direct consequence, central to this work, is that for a pure bipartite state, the entropy of the two reduced states coincides:
\begin{equation}
...
\begin{equation}
    S_{\text{vN}}(\rho) = -\operatorname{Tr}\bigl(\rho \log_2 \rho\bigr).
\end{equation}
...
\begin{equation}
    \label{eq:renyi2}
    S_2(\rho) = -\log_2 \operatorname{Tr}(\rho^2) = -\log_2 P_2(\rho),
\end{equation}
where $P_2(\rho) = \operatorname{Tr}(\rho^2)$ is the purity.
...
Given $N$ independent classical shadows $\{\hat{\rho}_i\}_{i=1}^{N}$ of a $k$-qubit state $\rho_A$, an unbiased estimator of the purity $\operatorname{Tr}(\rho_A^2)$ is the U-statistic of order two \cite{huang2020predicting, elben2020cross}:
\begin{equation}
    \label{eq:u_statistic}
    \widehat{\operatorname{Tr}}(\rho^2) = \frac{1}{\binom{N}{2}} \sum_{1 \leq i < j \leq N} \operatorname{Tr}\bigl(\hat{\rho}_i \hat{\rho}_j\bigr).
\end{equation}

\begin{theorem}[Unbiasedness]\label{thm:unbiasedness}
The estimator in \cref{eq:u_statistic} is unbiased: $\E\bigl[\widehat{\operatorname{Tr}}(\rho^2)\bigr] = \operatorname{Tr}(\rho^2)$.
\end{theorem}

\begin{proof}
For $i \neq j$, the snapshots $\hat{\rho}_i$ and $\hat{\rho}_j$ are independent, hence
\begin{equation}
    \E\bigl[\operatorname{Tr}(\hat{\rho}_i \hat{\rho}_j)\bigr] = \operatorname{Tr}\bigl(\E[\hat{\rho}_i] \E[\hat{\rho}_j]\bigr) = \operatorname{Tr}(\rho \cdot \rho) = \operatorname{Tr}(\rho^2).
\end{equation}
...
\begin{equation}
    ...
    \Var\bigl[\widehat{\operatorname{Tr}}(\rho^2)\bigr] \sim \frac{4^k}{N},
\end{equation}
...
The variance of a U-statistic with kernel $h(x,y) = \operatorname{Tr}(\hat{\rho}_x \hat{\rho}_y)$ is dominated by the first-order Hoeffding term \cite{hoeffding1948class}:
\begin{equation}
    \Var\bigl[\widehat{\operatorname{Tr}}(\rho^2)\bigr] = \frac{4(N-2)}{N(N-1)}\zeta_1 + \frac{2}{N(N-1)}\zeta_2,
\end{equation}
...
\begin{equation}
    ...
    \operatorname{Tr}(\hat{\rho}_i \hat{\rho}_j) = \prod_{\ell=1}^{k} \operatorname{Tr}\bigl(\hat{\rho}_i^{(\ell)} \hat{\rho}_j^{(\ell)}\bigr),
\end{equation}
...
\begin{equation}
    ...
    \operatorname{Tr}\bigl(\hat{\rho}_i^{(\ell)} \hat{\rho}_j^{(\ell)}\bigr) =
    \begin{cases}
...
\caption{Architecture of the calibration and validation protocol. The FPGA layer determines the operational noise threshold; the estimator layer implements the TRACE_CACHE-optimized U-statistic; the emulator layer produces noisy measurement data for the [[5,1,3]] code.}
\end{figure}
...
\Ensure Estimate $\widehat{P}_2 = \widehat{\operatorname{Tr}}(\rho^2)$
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig_circuit.png}
\caption{Encoding circuit for the [[5,1,3]] logical state |0_L>: Hadamard gates on qubits 0-3, CNOT gates to qubit 4, the Z0Z3 interaction, and three controlled-Z gates.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig_kl_curve.png}
\caption{KL divergence as a function of bit-flip probability. The operational threshold is identified at D_KL(p) = 1 bit, giving p_operate = 0.0306.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig_spectrum.png}
\caption{Renyi-2 entropy spectrum of the [[5,1,3]] code: theoretical values (squares) and emulated values with IBEX Q1 noise (circles). All estimates agree with theory within 0.15 bits.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig_convergence.png}
...
\end{equation}
Expanding $\rho = \frac12\bigl(\mathbb{I} + \vec{r} \cdot \vec{\sigma}\bigr)$ with Bloch vector $\vec{r}$, one finds $\mathcal{M}(\rho) = \frac13 \rho + \frac13 \cdot \frac{\operatorname{Tr}(\rho)}{2} \mathbb{I}$, i.e., $\mathcal{M}$ acts as $\mathcal{M} = \frac13 \mathbb{I} + \frac23 \mathcal{D}$ where $\mathcal{D}$ is the depolarizing channel. The inverse is $\mathcal{M}^{-1}(X) = 3X - \operatorname{Tr}(X)\mathbb{I}$, which applied to the single-qubit measurement outcome $|b\rangle\langle b|$ yields the snapshot $\hat{\rho} = 3|b\rangle\langle b| - \mathbb{I}$. This is the origin of the factor $3$ in \cref{eq:snapshot_pauli}.
...
\For{$b = 1$ \textbf{to} $B$}
    ...
    \State $\widehat{P}_2^{(b)} \gets \mathrm{EstimatePurity}(\{s^{(b)}_i\})$ via \cref{alg:estimator}
...
``Measuring $\operatorname{Tr}(\rho^n)$ on Single Copies of $\rho$ Using Random Measurements,''
