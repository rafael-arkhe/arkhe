...
# ==============================================================
# GEOMETRIA ADAPTATIVA (T-STAT 1.0, JANELA 15d)
# ==============================================================
...
class ArkheTrader:
    ...
    def run_day(self, prices_dict, rets_dict):
        ...
        if len(self.hist) < self.warmup:
            for s, p in prices_dict.items():
                if len(p) >= 20:
                    r = np.diff(np.log(p))[-20:]
                    new_pos[s] = np.clip((np.sum(r) / (np.std(r)+1e-8)) * 0.05, -0.2, 0.2)
                else: new_pos[s] = 0.0
        else:
            for s, z in self.state.items():
                ...
                for _ in range(3):
                    ...
                    z_cur = complex(*PoincareDisk.exponential_map(np.array([z_cur.real, z_cur.imag]), np.array([g.real, g.imag])*0.02))
                new_pos[s] = np.clip((self.geom.potential(z_cur) - self.geom.potential(z)) * 2.0, -0.3, 0.3)
        ...
        if len(self.hist) >= self.warmup and len(self.hist) % 5 == 0:
            for s in self.syms:
...
if __name__ == "__main__":
    ...
    trader = ArkheTrader(syms); dates = data.index
    print("Executando...")
    ...
    print("\n" + "="*50)
    print("RELATORIO FINAL - ARKHE TRADER REAL")
    print("="*50)
