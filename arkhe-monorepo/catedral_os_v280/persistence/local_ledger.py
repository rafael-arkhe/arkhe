...
"""
Ledger local com encadeamento por hash (fallback de L1).

Cada entrada e assinada (hash SHA-256), encadeada por `prev_hash` e
persistida como journal append-only (uma linha JSON por bloco). A integridade
da cadeia inteira pode ser verificada a qualquer momento (`verify_chain`).
O journal so cresce — append-only de verdade, Loopseal-2. Este e o fallback
honesto quando o IPFS nao esta disponivel — sem fingir distribuicao.
"""
...
class LocalLedger:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            # O cabeçalho e metadata (last_hash ali e o da criacao do journal);
            # a ancora real e a cauda da cadeia — evita continuar o encadeamento
            # a partir de um last_hash obsoleto.
            self.last_hash = (
