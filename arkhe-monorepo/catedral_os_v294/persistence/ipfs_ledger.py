...
"""
L1 — Ledger distribuído no IPFS (v294.0).

Encadeia cada entrada por hash (mesmas regras do ledger local) e, quando um
nó IPFS está acessível, publica cada bloco como objeto imutável no IPFS via
`ipfshttpclient`. O "last hash" local passa a ser o CID do bloco publicado;
o fallback local preserva a imutabilidade e a verificabilidade.

Nenhuma distribuição é fingida: sem `ipfshttpclient` ou sem daemon, o modo
local é explicitamente reportado em `stats().mode`.
"""
...
class IPFSLedger:
    """Ledger imutável distribuído com fallback local."""
    ...
    def __init__(self, gateway: str = DEFAULT_GATEWAY, use_ipfs: bool = True,
        ...
        if self.use_ipfs:
            ...
            except Exception as exc:  # pragma: no cover - rede indisponível
